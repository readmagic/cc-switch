...
"#,
    ...
    // Note: wezterm doesn't need the -e flag, others do
    let result = match terminal {
        "iterm2" => launch_macos_iterm2(&script_file),
        "alacritty" => launch_macos_open_app("Alacritty", &script_file, true),
        "wezterm" => launch_macos_open_app("wezterm", &script_file, false),
...
/// macOS: 使用 open -a 启动支持 --args 参数的终端（Alacritty/wezterm/Ghostty）
#[cfg(target_os = "macos")]
